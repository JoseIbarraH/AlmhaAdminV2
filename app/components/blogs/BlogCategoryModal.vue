...
const fetchCategories = async (selectNewIfAvailable = false) => {
  isLoadingCategories.value = true
  try {
    const response = await useApi<any>('/blog-categories', {
      headers: {
    ...
    toast.add({
      title: 'Error',
      description: t('blogs.categories.errorLoading', 'No se pudieron cargar las categorías.'),
      color: 'error'
    })
  } finally {
...
const saveCategory = async () => {
  ...
  try {
    ...
    const endpoint = isEdit ? `/blog-categories/${form.value.id}` : '/blog-categories'
    ...
    toast.add({
      title: 'Éxito',
      description: isEdit ? t('blogs.categories.successUpdate') : t('blogs.categories.successSave'),
      color: 'success'
    })
...
const deleteCategory = async (id: number) => {
  ...
  try {
    await useApi(`/blog-categories/${id}`, { method: 'DELETE' })
    toast.add({ title: t('blogs.categories.successDelete'), description: t('blogs.categories.successDelete'), color: 'success' })
    emit('updated')
        ...
        <aside class="w-full md:w-[350px] max-h-[220px] md:max-h-none border-b md:border-b-0 md:border-r border-slate-100 dark:border-slate-800 flex flex-col bg-slate-50/50 dark:bg-slate-900/50">
          <header class="p-6 pb-2">
            ...
            <div class="space-y-3">
              <div class="relative group">
                ...
                <UInput v-model="searchQuery" placeholder="Buscar..." variant="none" class="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 pl-10 rounded-xl w-full text-sm shadow-sm focus:border-gold" />
              </div>
              
              <UButton icon="i-heroicons-plus" color="primary" block size="md" rounded="xl" class="font-bold" @click="resetForm">
                {{ t('blogs.categories.newCategory') }}
              </UButton>
            </div>
          </header>
          ...
          <div class="flex-1 overflow-y-auto px-4 py-4 space-y-2 custom-scrollbar" v-auto-animate>
            ...
            <div v-else-if="filteredCategories.length === 0" class="text-center py-20 opacity-30">
              ...
              <p class="text-xs uppercase tracking-widest font-bold">Sin resultados</p>
            </div>
          ...
          <header class="p-6 border-b border-slate-50 dark:border-slate-800 flex justify-between items-center bg-white dark:bg-slate-900 sticky top-0 z-10">
            <div>
              <h2 class="text-2xl font-black text-slate-900 dark:text-white leading-tight">
                {{ form.id ? t('blogs.categories.editTitle') : t('blogs.categories.createTitle') }}
              </h2>
              <p class="text-slate-400 text-sm mt-1">{{ t('blogs.categories.formDescription') }}</p>
            </div>
            ...
            <div class="max-w-xl mx-auto space-y-12">
              ...
              <section class="form-vibe-section">
                <div class="flex items-center gap-2 mb-6">
                  ...
                  <h3 class="text-xs uppercase tracking-widest font-black text-slate-400">Datos Principales</h3>
                </div>

                <div class="space-y-8">
                  <div class="field-premium">
                    <label class="label-premium">{{ t('blogs.categories.labelName') }}</label>
                    <UInput v-model="form.title" placeholder="Ej: Noticias Médicas" size="xl" variant="none" class="input-premium" />
                    <p class="help-premium">{{ t('blogs.categories.helpName') }}</p>
                  </div>
